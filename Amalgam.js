...
async function parseCommand(message) {
	...
	switch(messageSplit[0]){
		...
		case "randomimg":
			...
			break;
		case "play":
			play(message, messageContent.substring("play ".length), streamMetadata, client);
			break;
		case "pause":
			pause(message, streamMetadata, client);
			break;
		case "resume":
			resume(message, streamMetadata, client);
			break;
		case "skip":
			skip(message, streamMetadata, client);
			break;
		case "stop":
			stop(message, streamMetadata, client);
			break;
		case "queue":
			queue(message, streamMetadata, client);
			break;
		case "furiganaize":
			furiganaize(message);
	}
...
client.on("voiceStateUpdate", (oldState, newState) => {
	try{
		if( newState.guild.member.me.voice.channel.members.size === 1 ){
			...
			newState.guild.member.me.voice.channel.leave();
		}
	}
	catch (err){
